...
def test_v103_version_and_weekly_ui_reload_guard():
    ...
    assert 'WEEKLY_UI_SCHEMA_VERSION = 8' in (ROOT / "weekly_ui.py").read_text("utf-8")
    assert 'getattr(_weekly_ui, "WEEKLY_UI_SCHEMA_VERSION", 0) < 8' in (ROOT / "app.py").read_text("utf-8")
