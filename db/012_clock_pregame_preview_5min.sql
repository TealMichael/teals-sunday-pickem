-- Teal's Sunday Pick'em v1.0.7-hotfix7.7
-- Sunday Pregame Clock Preview — 5-Minute Cadence
--
-- Adds a privacy-safe Sunday NFL schedule preview before the 1 PM ET lock.
-- 9:00 AM-noon ET: preview once every 5 minutes.
...
-- Safe to run after db/011_clock_pregame_preview.sql.
...
begin
  ...
  if now() < v_week.locks_at then
    ...
    if v_hour < 12 then
      -- 9:00-noon: one preview every five minutes. The five-minute event id
