-- Teal's Sunday Pick'em v1.0.7-hotfix7
-- Sunday Clock Rich Accents + Caleb 4K Watch
--
-- Uses AWTRIX colored-text fragments: most copy stays white while identity and
...
-- Safe to run once after db/008_clock_nfl_score_cadence.sql.
    ...
    case mod(v_test_slot, 5)
      when 0 then
        ...
        v_fragments := jsonb_build_array(
          jsonb_build_object('t', 'PICK''EM CLOCK TEST • ', 'c', '2DD4BF'),
          jsonb_build_object('t', 'CONNECTION OK', 'c', '34C759')
        );
      when 1 then
        v_text := 'WELCOME • SUNDAY CLOCK TEST';
        v_fragments := jsonb_build_array(
          jsonb_build_object('t', 'WELCOME • ', 'c', 'FF9F0A'),
          jsonb_build_object('t', 'SUNDAY CLOCK TEST', 'c', 'FFFFFF')
        );
      when 2 then
        v_text := 'PICK''EM LIVE • 1 PLAYER A 42.1 • 2 PLAYER B 39.8 • 3 PLAYER C 37.4';
        v_fragments := jsonb_build_array(
          jsonb_build_object('t', 'PICK''EM LIVE • ', 'c', '2DD4BF'),
          jsonb_build_object('t', '1 PLAYER A', 'c', 'FFD700'),
          jsonb_build_object('t', ' 42.1', 'c', '2DD4BF'),
          jsonb_build_object('t', ' • 2 PLAYER B 39.8 • 3 PLAYER C 37.4', 'c', 'FFFFFF')
        );
      when 3 then
        v_text := 'NFL LIVE TEST • CHI 21 • MIN 17 • Q3 4:22';
        v_fragments := jsonb_build_array(
          jsonb_build_object('t', 'NFL LIVE TEST • ', 'c', 'FF453A'),
          jsonb_build_object('t', 'CHI', 'c', 'F56600'),
          jsonb_build_object('t', ' 21 • ', 'c', 'FFFFFF'),
          jsonb_build_object('t', 'MIN', 'c', 'A78BFA'),
          jsonb_build_object('t', ' 17 • Q3 4:22', 'c', 'FFFFFF')
        );
      else
        v_text := '🐻 CALEB 4K WATCH 🐻 • SEASON: 2,941 YDS • 1,059 TO 4K • PACE: 4,165 👀 • BEARS FANS, DON''T JINX IT';
        v_fragments := jsonb_build_array(
          jsonb_build_object('t', '🐻 CALEB 4K WATCH 🐻 • ', 'c', 'F56600'),
          jsonb_build_object('t', 'SEASON: 2,941 YDS • ', 'c', 'FFFFFF'),
          jsonb_build_object('t', '1,059 TO 4K', 'c', '5B7CFA'),
          jsonb_build_object('t', ' • PACE: ', 'c', 'FFFFFF'),
          jsonb_build_object('t', '4,165 👀', 'c', '34C759'),
          jsonb_build_object('t', ' • BEARS FANS, DON''T JINX IT', 'c', 'F56600')
        );
    end case;
  ...
  if now() < v_week.locks_at then
    if mod(floor(v_minute / 10)::integer, 2) = 0 and array_length(v_manual, 1) is not null then
      ...
      if position(' • ' in v_text) > 0 then
        v_fragments := jsonb_build_array(
          jsonb_build_object('t', split_part(v_text, ' • ', 1) || ' • ', 'c', 'FF9F0A'),
          jsonb_build_object('t', substring(v_text from position(' • ' in v_text) + 3), 'c', 'FFFFFF')
        );
      end if;
    else
