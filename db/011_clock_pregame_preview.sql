-- Teal's Sunday Pick'em v1.0.7-hotfix7.6
-- Sunday Pregame Clock Preview
--
-- Adds a privacy-safe Sunday NFL schedule preview before the 1 PM ET lock.
-- 9:00 AM-noon ET: preview once every 10 minutes.
-- Noon-1:00 PM ET: preview and Welcome alternate every 5 minutes, so each
-- appears once every 10 minutes. If Welcome is disabled/blank, preview keeps
-- the slot. No picks, lineup ownership, standings, or storylines are exposed.
--
-- Core post-lock cadence is intentionally unchanged:
--   :00 weekly standings
--   :05 NFL LIVE scores
...
-- Safe to run after db/010_clock_rich_fragment_render_fix.sql.
...
declare
  ...
  v_preview_text text := '';
  v_preview_fragments jsonb := '[]'::jsonb;
...
begin
  ...
  -- Pick'em broadcast is Sunday-only: begin 9:00 AM ET and stop at midnight.
  select * into v_week
  from pickem.weeks
  where is_demo = false
    and now() >= locks_at - interval '4 hours'
  ...
  -- player-selection storylines. The pregame clock uses only the public NFL
  -- schedule plus the Commissioner's optional Welcome message.
  if now() < v_week.locks_at then
    -- Build one compact preview of every Sunday NFL game in this Pick'em week.
    -- The week cache can also contain Thu/Mon games, so match the local Sunday
    -- date of the universal lock explicitly.
    v_preview_text := 'SUNDAY NFL PREVIEW';
    v_preview_fragments := jsonb_build_array(
      jsonb_build_object('text', 'SUNDAY NFL PREVIEW • ', 'color', '64D2FF')
    );
    for v_game in
      select away_team, home_team, kickoff_at
      from pickem.nfl_games
      where week_id = v_week.id
        and (timezone('America/New_York', kickoff_at))::date = (timezone('America/New_York', v_week.locks_at))::date
        and game_status not in ('CANCELED', 'POSTPONED')
      order by kickoff_at, away_team, home_team
    loop
      if v_preview_count = 0 then
        v_preview_text := v_preview_text || ' • ';
      else
        v_preview_text := v_preview_text || ' • ';
        v_preview_fragments := v_preview_fragments || jsonb_build_array(
          jsonb_build_object('text', ' • ', 'color', 'FFFFFF')
        );
      end if;
      v_preview_text := v_preview_text
        || upper(v_game.away_team) || ' @ ' || upper(v_game.home_team) || ' '
        || to_char(timezone('America/New_York', v_game.kickoff_at), 'FMHH12:MI AM');
      v_preview_fragments := v_preview_fragments || jsonb_build_array(
        jsonb_build_object('text', upper(v_game.away_team), 'color', 'FFFFFF'),
        jsonb_build_object('text', ' @ ', 'color', 'FFFFFF'),
        jsonb_build_object('text', upper(v_game.home_team), 'color', 'FFFFFF'),
        jsonb_build_object('text', ' ' || to_char(timezone('America/New_York', v_game.kickoff_at), 'FMHH12:MI AM'), 'color', 'FFD60A')
      );
      v_preview_count := v_preview_count + 1;
    end loop;

    if v_preview_count = 0 then
      v_preview_text := coalesce(v_payload->>'readiness', 'SUNDAY PICK''EM • PICKS LOCK 1 PM ET');
      v_preview_fragments := coalesce(v_payload->'readiness_rich', '[]'::jsonb);
    end if;

    if v_hour < 12 then
      -- 9:00-noon: one preview every ten minutes. The ten-minute event id
      -- prevents the 15-second AWTRIX poll from replaying it between slots.
      v_slot := floor(extract(epoch from now()) / 600)::bigint;
      v_category := 'nfl_preview';
      v_text := v_preview_text;
      v_fragments := v_preview_fragments;
    else
      -- Noon-lock: Welcome at :00/:10/:20/... and preview at :05/:15/:25/... .
      -- That makes the noon transition begin with Welcome while keeping each
      -- message on its own ten-minute cadence. Welcome never exposes picks.
      v_slot := floor(extract(epoch from now()) / 300)::bigint;
      if mod(floor(v_minute / 5)::integer, 2) = 0
        and coalesce(v_settings.welcome_enabled, false)
        and btrim(coalesce(v_settings.welcome_text, '')) <> '' then
        v_category := 'welcome';
        v_text := 'WELCOME • ' || btrim(v_settings.welcome_text);
        v_fragments := jsonb_build_array(
          jsonb_build_object('text', 'WELCOME • ', 'color', 'FF9F0A'),
          jsonb_build_object('text', btrim(v_settings.welcome_text), 'color', 'FFFFFF')
        );
      else
        -- Odd five-minute slots are the preview. If Welcome is disabled/blank,
        -- preview also fills its slot rather than showing nothing.
        v_category := 'nfl_preview';
        v_text := v_preview_text;
        v_fragments := v_preview_fragments;
      end if;
